...
async function collectPackages(nodeModulesPath) {
    ...
    return [...records.values()].filter((record) => !isPlatformNativePackage(record.name)).sort((left, right) => left.name.localeCompare(right.name) || left.version.localeCompare(right.version));
}

function isPlatformNativePackage(name) {
    return /^@esbuild\//.test(name) || /^@oxlint\/(?:android|darwin|freebsd|linux|win32)/.test(name) || /^@rollup\/rollup-/.test(name) || /^@tailwindcss\/oxide-/.test(name) || /^lightningcss-(?:android|darwin|freebsd|linux|win32)/.test(name);
}
