...
export default function PromptsPage() {
    ...
    const refresh = async (signal?: AbortSignal) => {
        setLoading(true);
        try {
            const result = await fetchCreativePrompts({ page, pageSize, search: search.trim() || undefined, category: category || undefined, promptType: promptType || undefined, tag: tag.trim() || undefined, sourceType: sourceType || undefined, favorite, signal });
    ...
    const openEdit = (prompt: CreativePrompt) => {
        setEditing(prompt);
        form.setFieldsValue({
            title: prompt.title, content: prompt.content, negative_prompt: prompt.negative_prompt, prompt_type: prompt.prompt_type || "general", category: prompt.category || "general",
            tags: prompt.tags || [], variables: prompt.variables || [], model_suggestions: prompt.model_suggestions || [], cover_asset_id: prompt.cover_asset_id,
            author_name: prompt.author_name, source_type: prompt.source_type || "user", source_url: prompt.source_url, source_license: prompt.source_license,
            allowed_uses: prompt.allowed_uses, source_updated_at: prompt.source_updated_at,
        });
        setFormOpen(true);
    };
        ...
        <div className="flex h-full flex-col overflow-hidden bg-background text-stone-900 dark:text-stone-100">
            <main className="min-h-0 flex-1 overflow-y-auto bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] px-4 py-6 [background-size:16px_16px] sm:px-6 lg:py-8 dark:bg-[radial-gradient(rgba(245,245,244,.14)_1px,transparent_1px)]">
                <div className="mx-auto max-w-7xl">
                    ...
                    <div className="mt-8 flex justify-center"><Pagination current={page} pageSize={pageSize} total={total} showSizeChanger pageSizeOptions={[12, 24, 48, 96]} onChange={(next, size) => { setPage(next); setPageSize(size); }} /></div>
                </div>
            </main>
            ...
            <Modal title={editing ? "编辑提示词" : "新建提示词"} open={formOpen} onCancel={() => setFormOpen(false)} onOk={() => void save()} okText="保存" cancelText="取消" width={720} destroyOnHidden>
                <Form form={form} layout="vertical" requiredMark={false} className="pt-2">
                    <Form.Item name="title" label="标题" rules={[{ required: true, message: "请输入标题" }]}><Input maxLength={200} showCount /></Form.Item>
                    <Form.Item name="content" label="提示词正文" extra="可使用 {{subject}} 形式声明变量，保存时会自动识别。" rules={[{ required: true, message: "请输入提示词正文" }]}><Input.TextArea rows={9} maxLength={100000} showCount /></Form.Item>
                    <Form.Item name="negative_prompt" label="负向提示词"><Input.TextArea rows={3} maxLength={20000} /></Form.Item>
                    <div className="grid gap-4 sm:grid-cols-2"><Form.Item name="prompt_type" label="类型"><Select options={categoryOptions} /></Form.Item><Form.Item name="category" label="分类"><Select options={categoryOptions} /></Form.Item></div>
                    <div className="grid gap-4 sm:grid-cols-2"><Form.Item name="tags" label="标签"><Select mode="tags" tokenSeparators={[",", "，"]} maxCount={50} /></Form.Item><Form.Item name="model_suggestions" label="建议模型"><Select mode="tags" tokenSeparators={[",", "，"]} maxCount={20} /></Form.Item></div>
                    <div className="grid gap-4 sm:grid-cols-2"><Form.Item name="cover_asset_id" label="封面素材 ID"><Input maxLength={80} /></Form.Item><Form.Item name="source_type" label="来源"><Select options={[{ label: "我的创作", value: "user" }, { label: "文件导入", value: "import" }, { label: "外部来源", value: "external" }]} /></Form.Item></div>
                    {editingSourceType === "external" ? <>
                        <div className="grid gap-4 sm:grid-cols-2"><Form.Item name="author_name" label="作者" rules={[{ required: true, message: "请填写原作者" }]}><Input maxLength={120} /></Form.Item><Form.Item name="source_license" label="许可证" rules={[{ required: true, message: "请填写许可证" }]}><Input maxLength={120} /></Form.Item></div>
                        <Form.Item name="source_url" label="原始链接" rules={[{ required: true, type: "url", message: "请填写有效的原始链接" }]}><Input /></Form.Item>
                        <Form.Item name="allowed_uses" label="允许用途" rules={[{ required: true, message: "请说明允许用途" }]}><Input.TextArea rows={2} maxLength={2000} /></Form.Item>
                    </> : null}
                </Form>
            </Modal>

            <input ref={importInputRef} type="file" accept="application/json,.json" className="hidden" onChange={(event) => { void importPrompts(event.target.files?.[0]); event.target.value = ""; }} />

            <Modal title="AI 精修对比" open={Boolean(refining)} onCancel={() => { setRefining(undefined); setRefinedText(""); }} onOk={() => void acceptRefinement()} okText="采用并创建新版本" cancelText="回退" okButtonProps={{ disabled: refiningLoading || !refinedText.trim() }} width={920} destroyOnHidden>
                <div className="grid gap-4 pt-2 md:grid-cols-2">
                    <label><span className="mb-2 block text-sm font-medium">精修前</span><Input.TextArea value={refining?.content || ""} readOnly autoSize={{ minRows: 12, maxRows: 24 }} /></label>
                    <label><span className="mb-2 block text-sm font-medium">精修后（可修改）</span><Input.TextArea value={refinedText} onChange={(event) => setRefinedText(event.target.value)} placeholder={refiningLoading ? "正在通过 FYJIT Job 精修…" : "精修结果"} autoSize={{ minRows: 12, maxRows: 24 }} disabled={refiningLoading} /></label>
                </div>
            </Modal>

            <PromptDrawer prompt={selected} onClose={() => setSelected(undefined)} onCopy={copyText} onSaveAsset={async (prompt, rendered) => {
            ...
            <Modal title="删除提示词" open={Boolean(deleting)} onCancel={() => setDeleting(undefined)} onOk={() => {
                if (!deleting) return;
                const target = deleting;
                setDeleting(undefined);
                void runAction(() => deleteCreativePrompt(target.prompt_id), "提示词已删除");
            }} okText="删除" okButtonProps={{ danger: true }} cancelText="取消">确定删除「{deleting?.title}」吗？历史版本将保留在数据库审计记录中。</Modal>
        </div>
    );
}

function PromptCard({ prompt, onOpen, onEdit, onCopy, onFavorite, onDuplicate, onRefine, onDelete }: { prompt: CreativePrompt; onOpen: () => void; onEdit: () => void; onCopy: () => void; onFavorite: () => void; onDuplicate: () => void; onRefine: () => void; onDelete: () => void }) {
    return <Card hoverable className="overflow-hidden" styles={{ body: { padding: 0 } }}>
        <button type="button" className="block w-full p-4 text-left" onClick={onOpen}>
            <div className="flex items-start justify-between gap-3"><h2 className="line-clamp-1 text-sm font-semibold">{prompt.title}</h2><Tag className="m-0 shrink-0">v{prompt.version}</Tag></div>
            ...
            <div className="mt-3 flex flex-wrap gap-1">{prompt.prompt_type ? <Tag color="purple" className="m-0 text-[11px]">{categoryOptions.find((item) => item.value === prompt.prompt_type)?.label || prompt.prompt_type}</Tag> : null}{prompt.category ? <Tag color="blue" className="m-0 text-[11px]">{categoryOptions.find((item) => item.value === prompt.category)?.label || prompt.category}</Tag> : null}{prompt.source_type === "external" ? <Tag color="gold" className="m-0 text-[11px]">外部来源</Tag> : null}{(prompt.tags || []).slice(0, 3).map((tag) => <Tag key={tag} className="m-0 text-[11px]">{tag}</Tag>)}</div>
        </button>
        <div className="flex flex-wrap gap-2 px-4 pb-4">
            <Button size="small" icon={<Copy className="size-3.5" />} onClick={onCopy}>复制</Button>
            <Button size="small" icon={<Heart className="size-3.5" fill={prompt.favorite ? "currentColor" : "none"} />} onClick={onFavorite}>{prompt.favorite ? "已收藏" : "收藏"}</Button>
            <Button size="small" icon={<PencilLine className="size-3.5" />} onClick={onEdit}>编辑</Button>
            <Button size="small" icon={<Files className="size-3.5" />} onClick={onDuplicate}>副本</Button>
            <Button size="small" icon={<Sparkles className="size-3.5" />} onClick={onRefine}>AI 精修</Button>
...
function PromptDrawer({ prompt, onClose, onCopy, onSaveAsset }: { prompt?: CreativePrompt; onClose: () => void; onCopy: (value: string, message?: string) => void; onSaveAsset: (prompt: CreativePrompt, rendered: string) => Promise<void> }) {
    ...
    useEffect(() => {
        ...
        const controller = new AbortController();
        void fetchCreativePromptRevisions(prompt.prompt_id, controller.signal).then(setRevisions).catch(() => setRevisions([]));
        return () => controller.abort();
    }, [prompt]);

    return <Drawer title={prompt?.title || "提示词详情"} open={Boolean(prompt)} size="large" onClose={onClose}>
        {prompt ? <div className="space-y-5">
            <div className="flex flex-wrap gap-2"><Tag color="purple">{prompt.prompt_type || "general"}</Tag><Tag color="blue">{prompt.category || "general"}</Tag><Tag>版本 v{prompt.version}</Tag>{prompt.tags.map((tag) => <Tag key={tag}>{tag}</Tag>)}</div>
            {prompt.source_type === "external" ? <section className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-900 dark:bg-amber-950/20">
                ...
                {prompt.source_url ? <a className="mt-2 inline-flex items-center gap-1 text-blue-600 hover:underline" href={prompt.source_url} target="_blank" rel="noreferrer"><Link className="size-3.5" />查看原始来源</a> : null}
            </section> : null}
            {variables.length ? <section><Typography.Title level={5}>填写变量</Typography.Title><div className="grid gap-3 sm:grid-cols-2">{variables.map((variable) => <label key={variable.name} className="text-sm"><span className="mb-1 block text-stone-500">{variable.label || variable.name}{variable.required ? " *" : ""}</span><Input value={values[variable.name] || ""} onChange={(event) => setValues((current) => ({ ...current, [variable.name]: event.target.value }))} /></label>)}</div></section> : null}
            <section><Typography.Title level={5}>最终提示词</Typography.Title><pre className="whitespace-pre-wrap rounded-xl bg-stone-100 p-4 text-sm leading-7 dark:bg-stone-900">{rendered}</pre></section>
            {prompt.negative_prompt ? <section><Typography.Title level={5}>负向提示词</Typography.Title><p className="whitespace-pre-wrap text-sm text-stone-600 dark:text-stone-400">{prompt.negative_prompt}</p></section> : null}
            <Space wrap><Button type="primary" icon={<Copy className="size-4" />} onClick={() => onCopy(rendered, "最终提示词已复制")}>复制最终提示词</Button><Button onClick={() => void onSaveAsset(prompt, rendered)}>保存为文本素材</Button></Space>
            <section><Typography.Title level={5}><span className="inline-flex items-center gap-2"><History className="size-4" />版本历史</span></Typography.Title><div className="space-y-2">{revisions.map((revision) => <details key={revision.revision_id} className="rounded-lg border border-stone-200 p-3 dark:border-stone-800"><summary className="cursor-pointer text-sm font-medium">v{revision.version} · {formatTime(revision.created_at)}</summary><pre className="mt-3 whitespace-pre-wrap text-xs leading-6 text-stone-600 dark:text-stone-400">{revision.content}</pre></details>)}</div></section>
        </div> : null}
    </Drawer>;
}
