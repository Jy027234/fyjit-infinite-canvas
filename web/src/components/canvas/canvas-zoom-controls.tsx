...
export function CanvasZoomControls({ scale, onScaleChange, onReset, isMiniMapOpen, onToggleMiniMap }: CanvasZoomControlsProps) {
    ...
    return (
        <div className="absolute bottom-20 left-3 z-50 sm:bottom-5 sm:left-5" onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
            <div className="flex h-12 items-center gap-1 rounded-xl border px-1 shadow-lg backdrop-blur sm:h-14 sm:px-2" style={dockStyle}>
                <Tooltip title={isMiniMapOpen ? "关闭小地图" : "打开小地图"}>
                    <Button
                        type="text"
                        className="!h-11 !w-11 !min-w-11 !p-0 sm:!h-8 sm:!w-8 sm:!min-w-8"
                ...
                <Tooltip title="重置视图">
                    <Button type="text" className="!h-11 !w-11 !min-w-11 !p-0 sm:!h-8 sm:!w-8 sm:!min-w-8" style={{ color: theme.toolbar.item }} icon={<Focus className="size-4" />} onClick={onReset} aria-label="重置视图" />
                </Tooltip>
                ...
                <Tooltip title="快捷键">
                    <Button type="text" className="hidden !h-8 !w-8 !min-w-8 !p-0 sm:inline-flex" style={shortcutsOpen ? activeStyle : { color: theme.toolbar.item }} icon={<HelpCircle className="size-4" />} onClick={() => setShortcutsOpen(true)} aria-label="快捷键" />
                </Tooltip>
            </div>
