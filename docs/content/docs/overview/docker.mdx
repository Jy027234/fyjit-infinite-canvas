...
本镜像是 FYJIT 创作中心的独立发布单元。它需要由同域反向代理连接 FYJIT 主服务，不能作为浏览器直连模型的独立个人工具部署。
...
FYJIT_CREATIVE_IMAGE=ghcr.io/OWNER/REPOSITORY@sha256:DIGEST SOURCE_COMMIT=COMMIT SOURCE_URL=https://github.com/OWNER/REPOSITORY/tree/COMMIT docker compose up -d
```
...
本容器只提供 `/creative/` 静态应用和独立健康检查。画布、素材、提示词、生成记录、会话、模型能力和计费均由 FYJIT 主服务负责；浏览器不保存上游 AI API Key。
